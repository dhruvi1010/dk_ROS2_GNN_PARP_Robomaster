#include "rona_physical/waypoint_publisher.hpp"
      ...
      tf_listener_(tf_buffer_) 
{
    ...
    for (uint i = 0; i < poses_.size(); i++) {
        double heading;
        if (i == 0) {
            heading = atan2(poses_[i + 1].pose.position.y - poses_[poses_.size() - 1].pose.position.y,
                            poses_[i + 1].pose.position.x - poses_[poses_.size() - 1].pose.position.x);
        } else if (i == poses_.size() - 1) {
            heading = atan2(poses_[0].pose.position.y - poses_[i - 1].pose.position.y,
                            poses_[0].pose.position.x - poses_[i - 1].pose.position.x);
        } else {
            heading = atan2(poses_[i + 1].pose.position.y - poses_[i - 1].pose.position.y,
...
void WaypointPublisher::publish_waypoints()
{
    ...
    send_goal_options.goal_response_callback = [this](auto goal_handle) { this->goal_response_callback(goal_handle); };
    send_goal_options.feedback_callback = [this](auto goal_handle, auto feedback) { this->feedback_callback(goal_handle, feedback); };
    send_goal_options.result_callback = [this](auto result) { this->result_callback(result); };
...
void WaypointPublisher::goal_response_callback(GoalHandleNavigateThroughPoses::SharedPtr goal_handle)
{
    if (!goal_handle) {
