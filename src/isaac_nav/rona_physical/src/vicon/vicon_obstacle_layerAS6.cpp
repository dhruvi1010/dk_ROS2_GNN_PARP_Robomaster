...
class ViconObstacleLayerAS6 : public nav2_costmap_2d::CostmapLayer
{
public:
    void onInitialize() override
    {
        ros_node_ = rclcpp::Node::make_shared("vicon_obstacle_layerAS6");
        auto qos = rclcpp::QoS(rclcpp::KeepLast(10)).best_effort();

        declareParameter("topic", rclcpp::ParameterValue("/AS_6_neu/global_pose"));
        
        if (!ros_node_->get_parameter("topic", topic_) || topic_.empty())
        {
            topic_ = "/AS_6_neu/global_pose"; 
        ...
        sub_ = ros_node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            topic_, qos, std::bind(&ViconObstacleLayerAS6::obstacleCallback, this, std::placeholders::_1));
...
PLUGINLIB_EXPORT_CLASS(rona_physical::ViconObstacleLayerAS6, nav2_costmap_2d::Layer)
