...
class ViconObstacleLayerA : public nav2_costmap_2d::CostmapLayer
{
public:
    void onInitialize() override
    {
        ros_node_ = rclcpp::Node::make_shared("vicon_obstacle_layerA");
        auto qos = rclcpp::QoS(rclcpp::KeepLast(10)).best_effort();

        declareParameter("topic", rclcpp::ParameterValue("/AS_1_neu/global_pose"));
        
        if (!ros_node_->get_parameter("topic", topic_) || topic_.empty())
        {
            topic_ = "/AS_1_neu/global_pose"; 
        ...
        sub_ = ros_node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            topic_, qos, std::bind(&ViconObstacleLayerA::obstacleCallback, this, std::placeholders::_1));
...
PLUGINLIB_EXPORT_CLASS(rona_physical::ViconObstacleLayerA, nav2_costmap_2d::Layer)
