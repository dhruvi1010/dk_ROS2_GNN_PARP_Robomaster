...
private:
  ...
  unsigned char bandCostFromRsrp(double rsrp_dbm) const;

  // Sample storage — a sparse list of (wx, wy, risk, band_cost, stamp) tuples.
  struct Sample {
    double wx;
    double wy;
    double risk;             // [0, 1]
    unsigned char band_cost; // 0/green/yellow/red; 0 = no paint
    rclcpp::Time stamp;
  };

  // EMA-update the sample nearest to (wx, wy) within inflation radius, else append.
  void fuseSample(double wx, double wy, double risk, unsigned char band_cost, const rclcpp::Time & t);
...
// --- band-mode parameters (new) --------------------------------------
  bool   use_bands_{true};                  // false → keep old k_cost · risk formula
  double green_rsrp_max_dbm_{-84.0};        // rsrp >  this  → green band
  double yellow_rsrp_max_dbm_{-95.0};       // green_rsrp_max ≥ rsrp > yellow_rsrp_max → yellow
                                            // rsrp ≤ yellow_rsrp_max → red
  unsigned char green_cost_{30};            // 0..252; keep < 253 so inflation_layer ignores
  unsigned char yellow_cost_{150};
  unsigned char red_cost_{250};
