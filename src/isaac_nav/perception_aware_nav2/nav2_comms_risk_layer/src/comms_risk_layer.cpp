...
void CommsRiskLayer::onInitialize()
{
  ...
  declareParameter("decay_time_s", rclcpp::ParameterValue(15.0));   //in crease time to keep it visible for seconds
  declareParameter("only_raise", rclcpp::ParameterValue(true));
  // for band-mode (new)
  declareParameter("use_bands",            rclcpp::ParameterValue(true));
  declareParameter("green_rsrp_max_dbm",   rclcpp::ParameterValue(-84.0));
  declareParameter("yellow_rsrp_max_dbm",  rclcpp::ParameterValue(-95.0));
  declareParameter("green_cost",           rclcpp::ParameterValue(30));
  declareParameter("yellow_cost",          rclcpp::ParameterValue(150));
  declareParameter("red_cost",             rclcpp::ParameterValue(250));
  ...
  // for band-mode (new)
  node->get_parameter(name_ + ".use_bands",            use_bands_);
  node->get_parameter(name_ + ".green_rsrp_max_dbm",   green_rsrp_max_dbm_);
  node->get_parameter(name_ + ".yellow_rsrp_max_dbm",  yellow_rsrp_max_dbm_);
  int tmp;
  node->get_parameter(name_ + ".green_cost",  tmp); green_cost_  = static_cast<unsigned char>(std::clamp(tmp, 0, 252));
  node->get_parameter(name_ + ".yellow_cost", tmp); yellow_cost_ = static_cast<unsigned char>(std::clamp(tmp, 0, 252));
  node->get_parameter(name_ + ".red_cost",    tmp); red_cost_    = static_cast<unsigned char>(std::clamp(tmp, 0, 252));
...
// Map a raw rsrp_dbm to a band cost (0/30/150/250). Returns 0 if not finite.
unsigned char CommsRiskLayer::bandCostFromRsrp(double rsrp_dbm) const
{
  if (!std::isfinite(rsrp_dbm)) return 0;          // no reading → no paint
  if (rsrp_dbm >  green_rsrp_max_dbm_)  return green_cost_;
  if (rsrp_dbm >  yellow_rsrp_max_dbm_) return yellow_cost_;
  return red_cost_;                                 // rsrp ≤ yellow_rsrp_max_dbm_
}

// The main callback for incoming LinkStats messages. Converts to risk and fuses into the sample list.
  ...
  const perception_aware_nav2_msgs::msg::LinkStats::SharedPtr msg)
{
  ...
  double risk = riskFromStats(*msg);
  unsigned char band_cost = bandCostFromRsrp(msg->rsrp_dbm); // for band-mode (new)
  rclcpp::Time t = node->now();

  //fuseSample(rx, ry, risk, t);
  fuseSample(rx, ry, risk, band_cost, t);  // // signature gains one arg

  const char * band_name = (band_cost == red_cost_)    ? "RED"
                       : (band_cost == yellow_cost_) ? "YELLOW"
                       : (band_cost == green_cost_)  ? "GREEN"
                       : "none";
  RCLCPP_INFO_THROTTLE(node->get_logger(), *node->get_clock(), 3000,
  "\033[36m[CommsRiskLayer] sample @ (%.2f, %.2f) rsrp=%.1f dBm → %s (cost=%u) | risk=%.3f jit=%.1f ms\033[0m",
  rx, ry, msg->rsrp_dbm, band_name, band_cost, risk, msg->jitter_ms);
...
void CommsRiskLayer::fuseSample(
  double wx, double wy, double risk, unsigned char band_cost, const rclcpp::Time & t)

{
  ...
  if (nearest) {
    ...
    nearest->stamp = t;
    nearest->band_cost = band_cost;        // latest wins for band cost since it's not a smooth quantity like risk
  } else {
    samples_.push_back({wx, wy, risk, band_cost, t});
  }
  ...
  int min_i, int min_j, int max_i, int max_j)
{
  ...
  // (unchanged) skip if nothing to paint
  if (use_bands_) {
    if (s.band_cost == 0) continue;
  } else {
  ...
  for (int yj = yj_min; yj < yj_max; ++yj) {
    for (int xi = xi_min; xi < xi_max; ++xi) {
      ...
      if (use_bands_) {
        cell_cost = s.band_cost;     // FLAT band paint inside radius
      } else {
        double falloff = 1.0 - std::sqrt(d2) / r;
        double cell_cost_d = k_cost_ * s.risk * falloff;
        cell_cost = static_cast<unsigned char>(std::clamp(cell_cost_d, 0.0,
      ...
      // (unchanged) NEVER overwrite real obstacles or their inflation
