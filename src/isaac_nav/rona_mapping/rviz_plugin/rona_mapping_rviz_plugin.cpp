...
// Header
#include "rviz_plugin/rona_mapping_rviz_plugin.hpp"
// ROS
...
/*****************************************************************************/
{
  ...
  paused_measure = ros_node_->declare_parameter(
    "rona_mapping/paused_new_measurements", paused_measure);
  interactive = ros_node_->declare_parameter(
    "rona_mapping/interactive_mode", interactive);
  ...
  _serialize =
    ros_node_->create_client<rona_mapping::srv::SerializePoseGraph>(
    "rona_mapping/serialize_map");
  _load_map =
    ros_node_->create_client<rona_mapping::srv::DeserializePoseGraph>(
    "rona_mapping/deserialize_map");
  _clearChanges = ros_node_->create_client<rona_mapping::srv::Clear>(
    "rona_mapping/clear_changes");
  _saveChanges = ros_node_->create_client<rona_mapping::srv::LoopClosure>(
    "rona_mapping/manual_loop_closure");
  _saveMap = ros_node_->create_client<rona_mapping::srv::SaveMap>(
    "rona_mapping/save_map");
  _clearQueue = ros_node_->create_client<rona_mapping::srv::ClearQueue>(
    "rona_mapping/clear_queue");
  _interactive =
    ros_node_->create_client<rona_mapping::srv::ToggleInteractive>(
    "rona_mapping/toggle_interactive_mode");
  _pause_measurements = ros_node_->create_client<rona_mapping::srv::Pause>(
    "rona_mapping/pause_new_measurements");
  _load_submap_for_merging =
    ros_node_->create_client<rona_mapping::srv::AddSubmap>(
    "rona_mapping/add_submap");
  _merge = ros_node_->create_client<rona_mapping::srv::MergeMaps>(
    "rona_mapping/merge_submaps");
...
{
  auto request =
    std::make_shared<rona_mapping::srv::SerializePoseGraph::Request>();
...
/*****************************************************************************/
{
  typedef rona_mapping::srv::DeserializePoseGraph::Request procType;

  auto request =
    std::make_shared<rona_mapping::srv::DeserializePoseGraph::Request>();
...
/*****************************************************************************/
{
  auto request = std::make_shared<rona_mapping::srv::AddSubmap::Request>();
...
/*****************************************************************************/
{
  auto request = std::make_shared<rona_mapping::srv::MergeMaps::Request>();
...
/*****************************************************************************/
{
  auto request = std::make_shared<rona_mapping::srv::Clear::Request>();
...
/*****************************************************************************/
{
  auto request = std::make_shared<rona_mapping::srv::LoopClosure::Request>();
...
/*****************************************************************************/
{
  auto request = std::make_shared<rona_mapping::srv::SaveMap::Request>();
...
/*****************************************************************************/
{
  auto request = std::make_shared<rona_mapping::srv::ClearQueue::Request>();
...
{
  auto request =
    std::make_shared<rona_mapping::srv::ToggleInteractive::Request>();
...
/*****************************************************************************/
{
  auto request = std::make_shared<rona_mapping::srv::Pause::Request>();
...
/*****************************************************************************/
{
  ...
  auto parameters_client =
    std::make_shared<rclcpp::SyncParametersClient>(node, "rona_mapping");
  ...
  while (rclcpp::ok()) {
    ...
    if (parameters.empty()) {
      RCLCPP_INFO_THROTTLE(
        ...
        "Waiting for the rona_mapping node configuration.");
      log_trigger = true;
    } else {
      RCLCPP_INFO_EXPRESSION(
        ros_node_->get_logger(), log_trigger,
        "Start the rona_mapping node state check.");
      log_trigger = false;
...
PLUGINLIB_EXPORT_CLASS(rona_mapping::SlamToolboxPlugin, rviz_common::Panel)
