...
#include "rona_mapping/experimental/rona_mapping_lifelong.hpp"
...
TEST(LifelingMetricsTests, TestBounds)
{
  ...
  s1->SetIsDirty(dirty);
  s2->SetIsDirty(dirty);
  rona_mapping::LifelongSlamToolbox::computeIntersectBounds(s1, s2, x_l, x_u, y_l, y_u);
  EXPECT_EQ(x_l, 2.0);
  EXPECT_EQ(x_u, 5.0);
  EXPECT_EQ(y_l, 4.0);
  EXPECT_EQ(y_u, 6.0);
...
TEST(LifelingMetricsTests, TestIntersect)
{
  ...
  s1->SetIsDirty(dirty);
  s2->SetIsDirty(dirty);
  double intersect = rona_mapping::LifelongSlamToolbox::computeIntersect(s1, s2);
...
TEST(LifelingMetricsTests, TestIntersectOverUnion)
{
  ...
  double intersect_over_union =
    rona_mapping::LifelongSlamToolbox::computeIntersectOverUnion(s1, s2);
...
TEST(LifelingMetricsTests, TestAreaOverlap)
{
  ...
  s1->SetIsDirty(dirty);
  s2->SetIsDirty(dirty);
  double area = rona_mapping::LifelongSlamToolbox::computeAreaOverlapRatio(s1, s2);
...
TEST(LifelingMetricsTests, TestPtOverlap)
{
  ...
  s1->SetIsDirty(dirty);
  s2->SetIsDirty(dirty);
  double area = rona_mapping::LifelongSlamToolbox::computeReadingOverlapRatio(s1, s2);
  EXPECT_EQ(area, 0.5);
  delete s1;
  delete s2;
}
