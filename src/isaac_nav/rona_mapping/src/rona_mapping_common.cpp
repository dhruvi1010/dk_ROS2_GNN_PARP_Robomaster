...
#include <string>
#include <chrono>
#include "rona_mapping/rona_mapping_common.hpp"
#include "rclcpp/rclcpp/qos.hpp"
#include "rona_mapping/serialization.hpp"
...
: rclcpp_lifecycle::LifecycleNode("rona_mapping", "", options),
  solver_loader_("rona_mapping", "karto::ScanSolver"),
...
/*****************************************************************************/
{
  ...
  ssMap_ = this->create_service<nav_msgs::srv::GetMap>("rona_mapping/dynamic_map",
      std::bind(&SlamToolbox::mapCallback, this, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3));
  ssPauseMeasurements_ = this->create_service<rona_mapping::srv::Pause>(
    "rona_mapping/pause_new_measurements",
    std::bind(&SlamToolbox::pauseNewMeasurementsCallback,
    this, std::placeholders::_1,
    std::placeholders::_2, std::placeholders::_3));
  ssSerialize_ = this->create_service<rona_mapping::srv::SerializePoseGraph>(
    "rona_mapping/serialize_map",
    std::bind(&SlamToolbox::serializePoseGraphCallback, this,
    std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
  ssDesserialize_ = this->create_service<rona_mapping::srv::DeserializePoseGraph>(
    "rona_mapping/deserialize_map",
    std::bind(&SlamToolbox::deserializePoseGraphCallback, this,
    std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
  ssReset_ = this->create_service<rona_mapping::srv::Reset>(
    "rona_mapping/reset",
...
/*****************************************************************************/
{
  ...
  if (shouldStartWithPoseGraph(filename, pose, dock)) {
    std::shared_ptr<rona_mapping::srv::DeserializePoseGraph::Request> req =
      std::make_shared<rona_mapping::srv::DeserializePoseGraph::Request>();
    std::shared_ptr<rona_mapping::srv::DeserializePoseGraph::Response> resp =
      std::make_shared<rona_mapping::srv::DeserializePoseGraph::Response>();
    req->initial_pose = pose;
    req->filename = filename;
    if (dock) {
      req->match_type =
        rona_mapping::srv::DeserializePoseGraph::Request::START_AT_FIRST_NODE;
    } else {
      req->match_type =
        rona_mapping::srv::DeserializePoseGraph::Request::START_AT_GIVEN_POSE;
    }
...
bool SlamToolbox::pauseNewMeasurementsCallback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  const std::shared_ptr<rona_mapping::srv::Pause::Request> req,
  std::shared_ptr<rona_mapping::srv::Pause::Response> resp)
/*****************************************************************************/
{
...
bool SlamToolbox::serializePoseGraphCallback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  const std::shared_ptr<rona_mapping::srv::SerializePoseGraph::Request> req,
  std::shared_ptr<rona_mapping::srv::SerializePoseGraph::Response> resp)
/*****************************************************************************/
{
...
bool SlamToolbox::deserializePoseGraphCallback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  const std::shared_ptr<rona_mapping::srv::DeserializePoseGraph::Request> req,
  std::shared_ptr<rona_mapping::srv::DeserializePoseGraph::Response> resp)
/*****************************************************************************/
{
  if (req->match_type == rona_mapping::srv::DeserializePoseGraph::Request::UNSET) {
...
bool SlamToolbox::resetCallback(
  const std::shared_ptr<rmw_request_id_t> request_header,
  const std::shared_ptr<rona_mapping::srv::Reset::Request> req,
  std::shared_ptr<rona_mapping::srv::Reset::Response> resp)
/*****************************************************************************/
{
  boost::mutex::scoped_lock lock(smapper_mutex_);
  // Reset the map.
