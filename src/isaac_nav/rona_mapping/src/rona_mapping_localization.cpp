...
#include <memory>
#include <string>
#include "rona_mapping/rona_mapping_localization.hpp"
...
/*****************************************************************************/
{
  ...
  if (shouldStartWithPoseGraph(filename, pose, dock)) {
    ...
    req->match_type =
      rona_mapping::srv::DeserializePoseGraph::Request::LOCALIZE_AT_POSE;
    if (dock) {
...
/*****************************************************************************/
{
  ...
  clear_localization_ = this->create_service<std_srvs::srv::Empty>(
    "rona_mapping/clear_localization_buffer",
    std::bind(
...
RCLCPP_COMPONENTS_REGISTER_NODE(rona_mapping::LocalizationSlamToolbox)
