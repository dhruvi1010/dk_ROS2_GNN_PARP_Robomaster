...
![rona_mapping_sync_diagram](/images/rona_mapping_sync.png)
...
*multirobot_rona_mapping_node* extends rona_mapping for multi-robot mapping and localization. Slam_toolbox instances are run on each robot under unique namespaces and pose graph data are shared between the nodes through */localized_scan* topic. Each rona_mapping instance publishes transform of host robot and peers in the network. Peer transform frame names are prefixed with the namespace of the peer.
...
In two seperate terminals, launch two *multirobot_rona_mapping_node* instances for robot1 and robot2.

```shell
ros2 launch rona_mapping online_async_multirobot_launch.py namespace:=robot1
ros2 launch rona_mapping online_async_multirobot_launch.py namespace:=robot2
```
...
| Topic  | Type | Description | 
|-----|----|----|
| `/rona_mapping/clear_changes`  | `rona_mapping/Clear` | Clear all manual pose-graph manipulation changes pending | 
| `/rona_mapping/deserialize_map`  | `rona_mapping/DeserializePoseGraph` | Load a saved serialized pose-graph files from disk | 
| `/rona_mapping/dynamic_map`  | `nav_msgs/OccupancyGrid` | Request the current state of the pose-graph as an occupancy grid | 
| `/rona_mapping/manual_loop_closure`  | `rona_mapping/LoopClosure` | Request the manual changes to the pose-graph pending to be processed | 
| `/rona_mapping/pause_new_measurements`  | `rona_mapping/Pause` | Pause processing of new incoming laser scans by the toolbox | 
| `/rona_mapping/save_map`  | `rona_mapping/SaveMap` | Save the map image file of the pose-graph that is useable for display or AMCL localization. It is a simple wrapper on `map_server/map_saver` but is useful. | 
| `/rona_mapping/serialize_map`  | `rona_mapping/SerializePoseGraph` | Save the map pose-graph and datathat is useable for continued mapping, rona_mapping localization, offline manipulation, and more | 
| `/rona_mapping/toggle_interactive_mode`  | `rona_mapping/ToggleInteractive` | Toggling in and out of interactive mode, publishing interactive markers of the nodes and their positions to be updated in an application | 
| `/rona_mapping/reset`  | `rona_mapping/Reset` | Reset current map back to the initial state | 
...
`localization_on_configure` - Set to true to set the localization mode to localization during node on_configure transition. Set to false to set the localization mode to mapping instead. Only applies to `map_and_localization_rona_mapping` node.
...
You can run via `ros2 launch rona_mapping online_sync_launch.py`
