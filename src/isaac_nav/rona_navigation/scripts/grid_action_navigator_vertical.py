...
class GridActionNavigator(Node):
    def __init__(self):
        ...
        self.waypoints = self.generate_grid_waypoints(x_start=7, x_end=-7, y_start=4.9, y_end=-3.0, step=0.5)
    ...
    def generate_grid_waypoints(self, x_start, x_end, y_start, y_end, step):
        """Generate waypoints for vertical grid-like path."""
        waypoints = []
        x = float(x_start)  # Start at the leftmost X
        direction = -1  # Start moving from +y to -y
        while x >= x_end:
            if direction == -1:  # Moving from +y to -y
                waypoints.append({'x': x, 'y': float(y_start)})
                waypoints.append({'x': x, 'y': float(y_end)})
            else:  # Moving from -y to +y
                waypoints.append({'x': x, 'y': float(y_end)})
                waypoints.append({'x': x, 'y': float(y_start)})
            # Move to the next x-level
            x -= step
            # Switch direction for the next column
            direction *= -1
        return waypoints
