...
def generate_launch_description():
    ...
    mapping_share = get_package_share_directory('rona_mapping')
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        ...
        #default_value=os.path.join(bringup_share, 'config', 'robomaster_nav2_fast_nvblox.yaml'),# write down nav2_params for a slow velocity 
    ...
    declare_map_yaml_cmd = DeclareLaunchArgument(
        'map',
        default_value=os.path.join(bringup_share, 'maps', 'zft_hall_12_june.yaml'),
    ...
    map_dir = LaunchConfiguration(
        'map',
        default=os.path.join(get_package_share_directory('rona_navigation'), 'maps', 'zft_hall_12_june.yaml'))
    ...
    slam_launch_cmd = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(mapping_share, 'launch','online_async_multirobot_launch.py')),
            launch_arguments={
                'namespace': prefix}.items())

    scan_cmd = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(bringup_share, 'launch','laserscan_filter.launch.py')),
            launch_arguments={
                'namespace': prefix}.items()
            )
    ...
    bringup_cmd_group = GroupAction([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(bringup_share, 'launch', 'rviz','navigation2_launch.py')),
            launch_arguments={
                'namespace': prefix,
